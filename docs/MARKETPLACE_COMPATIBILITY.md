...
This repository ships a standalone MCP server (`ansible-devtools-mcp`) and is compatible with VS Code MCP configuration today. This document summarizes official compatibility points and practical guidance for Marketplace distribution.
...
`ansible-devtools-mcp install --client copilot` writes the required server entry automatically.
...
- optional diagnostics (verify `ansible-devtools-mcp` is installed and runnable)

## Recommended approach

1. Keep `ansible-devtools-mcp` as the runtime server distributed via PyPI.
2. Optionally create a thin VS Code companion extension for onboarding UX.
3. In extension docs, clearly state runtime dependency (`ansible-devtools-mcp` executable available in PATH).
